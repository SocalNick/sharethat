 ...
 * @package    Scn_Auth
 * @subpackage Adapter
 * @copyright  Copyright (c) Nicholas Calugar (https://github.com/njcalugar/)
 * @license    https://github.com/njcalugar/SocalNick-PHP-Library/blob/master/LICENSE
 */

//namespace Scn\Auth\Adapter;

//use Scn\Auth\Exception,
...
class Scn_Auth_Adapter_MongoCollection implements Zend_Auth_Adapter_Interface
{
    ...
    /**
     * @var Doctrine\ODM\MongoDB\Query
     */
    protected $_query = null;

    /**
     * $_collectionName - the collection name to check
     *
     * @var string
     */
    ...
    /**
     * $_credentialField - field to be used as the credential
     *
     * @var string
     */
    ...
    /**
     * $_resultDocument - Results of mongo authentication query
     *
     * @var array
     */
     ...
     *
     * @param  DocumentManager			$dm 						If null, default dm assumed
     ...
     * @return void
     */
    public function __construct(DocumentManager $dm = null, $collectionName = null, $identityField = null,
                                $credentialField = null, $credentialTreatment = null)
    {
        $this->_setDocumentManager($dm);
        ...
        if (null !== $identityField) {
            $this->setIdentityColumn($identityField);
        }

        if (null !== $credentialField) {
            $this->setCredentialColumn($credentialField);
        }
    ...
    /**
     * _setDocumentManager() - set the document manager to be used for querying
     *
     * @param DocumentManager
     * @throws NoDocumentManagerException
     * @return Zend_Auth_Adapter_DbTable
     */
    protected function _setDocumentManager(DocumentManager $dm = null)
    {
        $this->_dm = $dm;
        ...
        if(null === $this->_dm) {
            ...
            $this->_dm = Zend_Db_Table_Abstract::getDefaultAdapter();
            if (null === $this->_dm) {
                throw new Exception\NoDocumentManagerException('This auth adapter requires a Document Manager');
            }
        }
...
//            $this->_dbSelect = $this->_dm->select();
//        }
//
...
//            $zendAuthCredentialMatchColumn = $this->_dm->foldCase('zend_auth_credential_match');
...
//            '(CASE WHEN ' .
//            $this->_dm->quoteInto(
//                $this->_dm->quoteIdentifier($this->_credentialColumn, true)
//                . ' = ' . $this->_credentialTreatment, $this->_credential
//                )
//            . ' THEN 1 ELSE 0 END) AS '
//            . $this->_dm->quoteIdentifier(
//                $this->_dm->foldCase('zend_auth_credential_match')
//                )
//            );
//
//        // get select
...
//                 ->where($this->_dm->quoteIdentifier($this->_identityColumn, true) . ' = ?', $this->_identity);
//
//        return $dbSelect;
//    }
//
...
//    {
//        try {
//            if ($this->_dm->getFetchMode() != Zend_DB::FETCH_ASSOC) {
//                $origDbFetchMode = $this->_dm->getFetchMode();
//                $this->_dm->setFetchMode(Zend_DB::FETCH_ASSOC);
//            }
//            $resultIdentities = $this->_dm->fetchAll($dbSelect->__toString());
//            if (isset($origDbFetchMode)) {
//                $this->_dm->setFetchMode($origDbFetchMode);
